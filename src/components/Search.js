...
export default function Search(props) {
    const { eventData, setSelectedEvent, setReRenderMarkers } =
        useMainContext();
    ...
    const userSearch = (searchQuery, eventData) => {
        ...
        if (searchQuery.length > 0 && filterdEventData) {
            for (const event in filterdEventData) {
            ...
            if (eventMatch.length === 0) {
                eventMatch = [
                    { title: 'No Results!', categories: [{ title: '' }] }
                ];
            }
                ...
                <input
                    ...
                    onKeyUp={() => {
                        ...
                        setTimeout(() => {
                            if (
                                searchQuery ===
                                searchBox.current.value.toLowerCase()
                            ) {
